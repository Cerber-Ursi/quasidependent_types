...
```
И теперь, наконец-то, можем написать первый реальный тест:
```rust
fn summing() {
    let (_, v1) = vect!(vec![1]);
    let (_, v2) = vect!(vec![1]);
...
Во-вторых - вернёмся снова к нашим натуральным числам.
...
<spoiler title="Лирическое отступление: ловушка переполнения">
...
```
В принципе, получившийся итератор корректен. Однако он не является fused-итератором (вменяемого перевода этого понятия я, к сожалению, не нашёл) - иначе говоря, не гарантирует, что он продолжит возвращать `None` после того, как вернул его один раз. Проверяется это в данном случае вот таким тестом:
```rust
#[test]
fn overflow() {
...
```
После такого дополнения тест практически мгновенно стал выдавать ошибку, ну а после исправления в `next`, соответственно, так же практически мгновенно завершаться без ошибки.
...
На практике все эти попытки очень сильно усложняются правилами по реализации типажей (конкретные примеры встретятся ниже), но кое-какие результаты мы всё-таки можем получить даже в имеющихся рамках.
...
Вариант третий, который в итоге и был реализован, требует снова вернуться в `qd_core`.
...
Абсолютно аналогично мы можем ввести понятие ассоциативности сложения:
```rust
...
```
И, используя три полученных доказательства, напишем вот такой тест:
```rust
// Пробная функция, которая принимает два числа, только если они доказанно равны.
...
Впрочем, чуть более сложные проверки становятся уже не особо-то решаемыми - полученная нами система всё-таки не насколько умная. Так, к примеру, вот такой вызов требует явно указать логическую цепочку (из двух шагов):
```rust
#[test]
fn proved() {
    // пропуск
...
```
А вот в этом случае требуется явно указать одно из правил в цепочке:
```rust
#[test]
fn proved() {
    // пропуск
